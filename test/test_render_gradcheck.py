...
if ENABLE_TORCH_CHECK:
    ...
    print(
        ...
        grid_density_grad_s.min(),
        grid_density_grad_s.max(),
    )
    print(
        "g_torch sigma min/max\n",
        grid_density_grad_t.min(),
        grid_density_grad_t.max(),
    )
    if grid.basis_type == svox2.BASIS_TYPE_3D_TEXTURE:
        print(
            "g_ours basis min/max\n",
            grid_basis_grad_s.min(),
            grid_basis_grad_s.max(),
        )
        print(
            "g_torch basis min/max\n",
            grid_basis_grad_t.min(),
            grid_basis_grad_t.max(),
        )
    if grid.use_background:
        print(
