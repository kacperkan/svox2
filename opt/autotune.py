...
def run_exp(
    ...
    if eval_mode and path.isfile(ckpt_path):
        ...
        print(opt_cmd)
        try:
            opt_ret = subprocess.check_output(
                opt_cmd, shell=True, env=env
...
def create_prodvars(variables, noise_stds={}):
    ...
    variables = {
        varname: auto_list(variables[varname]) for varname in variables
    }
    print("variables (prod)", variables)
    varnames = list(variables.keys())
    noise_stds = np.array(
        [noise_stds.get(varname, 0.0) for varname in varnames]
    )
    variables = [
        [(i, val) for val in variables[varname]]
...
if __name__ == "__main__":
    ...
    for task_templ in all_tasks_templ:
        for variables in prodvars:
            ...
            task["train_dir"] = path.join(
                train_root, task["train_dir"]
            )  # Required
            task["data_dir"] = path.join(
                data_root, task.get("data_dir", "")
            ).rstrip("/")
            task["flags"] = task.get("flags", []) + base_flags
            task["eval_flags"] = task.get("eval_flags", []) + base_eval_flags
            task["common_flags"] = (
                task.get("common_flags", []) + base_common_flags
            )
            task["config"] = task.get("config", default_config)
            os.makedirs(task["train_dir"], exist_ok=True)
            # santity check
            assert path.exists(task["train_dir"]), (
                task["train_dir"] + " does not exist"
            )
            assert path.exists(task["data_dir"]), (
                task["data_dir"] + " does not exist"
            )
    ...
    if args.eval:
        ...
        with open(leaderboard_path, "w") as leaderboard_file:
            ...
            for task in all_tasks:
                ...
                psnr_file_path = path.join(
                    train_dir, "test_renders", "psnr.txt"
                )
                ssim_file_path = path.join(
                    train_dir, "test_renders", "ssim.txt"
                )
                lpips_file_path = path.join(
                    train_dir, "test_renders", "lpips.txt"
                )
            ...
            if len(all_psnr):
                lines.append(
                    "Average PSNR: "
                    + str(sum(all_psnr) / len(all_psnr))
                    + "\n"
                )
            if len(all_ssim):
                lines.append(
                    "Average SSIM: "
                    + str(sum(all_ssim) / len(all_ssim))
                    + "\n"
                )
            if len(all_lpips):
                lines.append(
                    "Average LPIPS: "
                    + str(sum(all_lpips) / len(all_lpips))
                    + "\n"
                )
            if len(all_times):
                lines.append(
