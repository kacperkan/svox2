...
class NeRFDataset(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(
            "WARNING: This data loader is ONLY intended for use with NeRF-synthetic Blender data!!!!"
        )
        print(
            "If you want to try running this code on Instant-NGP data please use scripts/ingp2nsvf.py"
        )

        j = json.load(open(data_json, "r"))

        # OpenGL -> OpenCV
        cam_trans = torch.diag(
            torch.tensor([1, -1, -1, 1], dtype=torch.float32)
        )

        for frame in tqdm(j["frames"]):
            fpath = path.join(
                data_path, path.basename(frame["file_path"]) + ".png"
            )
            ...
            if scale < 1.0:
                ...
                im_gt = cv2.resize(
                    im_gt, (rsz_w, rsz_h), interpolation=cv2.INTER_AREA
                )
