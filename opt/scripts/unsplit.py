        ...
        for d in all_dirs:
            if d.startswith(pfx.name):
                if d == "pose":
                    dir_idx = len(dirs)
                dirs.append(
                    Dir(name=osp.join(base, d), valid_exts=pfx.valid_exts)
                )
    return dirs, dir_idx
...
refdir = dirs[dir_idx]
print(
    "going to unsplit", [x.name for x in dirs], "reference", dirs[dir_idx].name
)
        ...
        for filename in files:
            ...
            if ext.lower() not in dir_obj.valid_exts:
                print(
                    "SKIP ",
                    full_filename,
                    " Since it has an unsupported extension",
                )
                continue
