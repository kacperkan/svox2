...
def read_colmap(strPath):
    ...
    objIntrinsics = read_write_model.read_cameras_binary(
        strPath + "/cameras.bin"
    )[1]
    ...
    for intImage, objImage in enumerate(
        ...
        if objIntrinsics.model == "SIMPLE_RADIAL":
            objCameras[objImage.name] = {
                ...
                "intPoints": [
                    intPoint
                    for intPoint in objImage.point3D_ids
                    if intPoint != -1
                ],
            }
    ...
    for intPoint, objPoint in enumerate(
        read_write_model.read_points3D_binary(
            strPath + "/points3D.bin"
        ).values()
    ):
        objPoints.append(
            {
...
def generate_masks(vid_root, args, overwrite=False):
    ...
    for file_ind, file in enumerate(tqdm(files, desc=f"masks: {vid_name}")):
        ...
        humans_tens = (
            torch.FloatTensor(im.shape[0], im.shape[1]).fill_(1.0).to(device)
        )

        obj_predictions = maskrnn_model(
            [
                torch.FloatTensor(im.transpose(2, 0, 1) / 255.0)[
                    [2, 0, 1], :, :
                ].to(device)
            ]
        )[0]
        ...
        for mask_ind in range(obj_predictions["masks"].size(0)):
            if obj_predictions["scores"][mask_ind].item() > 0.5:
                if obj_predictions["labels"][mask_ind].item() == 1:
                    humans_tens[
                        obj_predictions["masks"][mask_ind, 0, :, :] > 0.5
                    ] = 0.0
...
def resize_frames(vid_root, args):
    ...
    for file_ind, file in enumerate(tqdm(files, desc=f"imresize: {vid_name}")):
        ...
        if im.shape[1] > args.max_width or im.shape[0] > args.max_height:
            factor = max(
                im.shape[1] / args.max_width, im.shape[0] / args.max_height
            )
...
def render_movie(vid_root, args):
    ...
    moviepy.editor.ImageSequenceClip(
        sequence=debug_dir, fps=25
    ).write_videofile(movie_fn, ffmpeg_params=ffmpeg_params)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max-width", type=int, default=1280, help="max image width"
    )
    parser.add_argument(
        "--max-height", type=int, default=768, help="max image height"
    )
    parser.add_argument(
        "--undistorted-output",
        default="images",
