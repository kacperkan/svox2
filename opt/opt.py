...
group.add_argument(
    "--lr_sigma_delay_mult", type=float, default=1e-2
)  # 1e-4)#1e-4)
...
group.add_argument(
    "--save_every", type=int, default=5, help="save every x epochs"
)
group.add_argument(
    "--eval_every", type=int, default=1, help="evaluate every x epochs"
)
...
group.add_argument(
    "--lambda_tv_lumisphere", type=float, default=0.0
)  # 1e-2)#1e-3)
...
assert (
    args.lr_sigma_final <= args.lr_sigma
), "lr_sigma must be >= lr_sigma_final"
assert args.lr_sh_final <= args.lr_sh, "lr_sh must be >= lr_sh_final"
assert (
    args.lr_basis_final <= args.lr_basis
), "lr_basis must be >= lr_basis_final"
...
grid.density_data.data[:] = (
    0.0 if args.lr_fg_begin_step > 0 else args.init_sigma
)
...
if grid.basis_type == svox2.BASIS_TYPE_3D_TEXTURE:
    ...
    optim_basis_mlp = torch.optim.Adam(
        grid.basis_mlp.parameters(), lr=args.lr_basis
    )
...
while True:
    ...
    def eval_step():
        ...
        with torch.no_grad():
            ...
            if (
                ...
                eq_dirs = (
                    torch.from_numpy(eq_dirs).to(device=device).view(-1, 3)
                )
                ...
                sphfuncs_cmapped = [
                    viridis_cmap(sphfunc) for sphfunc in sphfuncs
                ]
    ...
    def train_step():
        print("Train step")
        pbar = tqdm(
            enumerate(range(0, epoch_size, args.batch_size)),
        ...
        for iter_id, batch_begin in pbar:
            ...
            lr_basis = (
                lr_basis_func(gstep_id - args.lr_basis_begin_step)
                * lr_basis_factor
            )
            lr_sigma_bg = (
                lr_sigma_bg_func(gstep_id - args.lr_basis_begin_step)
                * lr_basis_factor
            )
            lr_color_bg = (
                lr_color_bg_func(gstep_id - args.lr_basis_begin_step)
                * lr_basis_factor
            )
            ...
            if (iter_id + 1) % args.print_every == 0:
                ...
                for stat_name in stats:
                    ...
                    summary_writer.add_scalar(
                        stat_name, stat_val, global_step=gstep_id
                    )
                    stats[stat_name] = 0.0
                ...
                summary_writer.add_scalar(
                    "lr_sigma", lr_sigma, global_step=gstep_id
                )
                if grid.basis_type == svox2.BASIS_TYPE_3D_TEXTURE:
                    summary_writer.add_scalar(
            ...
            if args.lambda_l2_sh > 0.0:
                grid.inplace_l2_color_grad(
                    grid.sh_data.grad, scaling=args.lambda_l2_sh
                )
    ...
    if (gstep_id_base - last_upsamp_step) >= args.upsamp_every:
        ...
        if reso_id < len(reso_list) - 1:
            ...
            grid.resample(
                ...
                weight_thresh=args.weight_thresh / z_reso
                if use_sparsify
                else 0.0,
