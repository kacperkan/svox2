...
class CubemapCoord:
    ...
    def query_in(self, cubemap: torch.Tensor):
        ...
        if cubemap.ndim == 4:
            ...
            return cubemap[
                torch.arange(face.size(0), device=face.device),
                face,
                self.u,
                self.v,
            ]
...
def eval_sg_at_dirs(
    sg_lambda: torch.Tensor, sg_mu: torch.Tensor, dirs: torch.Tensor
):
    """
...
def posenc(
    ...
    if cutoff < 1.0:
        ...
        four_feat = four_feat.view(
            shapeb[:-1] + [2, scales.shape[0], x.shape[-1]]
        )
