...
class RenderOptions:
    ...
    background_brightness: float = (
        1.0  # [0, 1], the background color black-white
    )

    step_size: float = (
        0.5  # Step size, in normalized voxels (not used for svox1)
    )
    #  (i.e. 1 = 1 voxel width, different from svox where 1 = grid width!)

    sigma_thresh: float = (
        1e-10  # Voxels with sigmas < this are ignored, in [0, 1]
    )
    ...
    last_sample_opaque: bool = (
        False  # Make the last sample opaque (for forward-facing)
    )
    ...
    random_sigma_std: float = (
        1.0  # Noise to add to sigma (only if randomize=True)
    )
...
class Camera:
    ...
    def gen_rays(self) -> Rays:
        ...
        origins = (
            self.c2w[None, :3, 3]
            .expand(self.height * self.width, -1)
            .contiguous()
        )
        yy, xx = torch.meshgrid(
            ...
            + 0.5,
            torch.arange(
                self.width, dtype=torch.float64, device=self.c2w.device
            )
            + 0.5,
        )
...
class SparseGrid(nn.Module):
    ...
    def _volume_render_gradcheck_lerp(
        ...
        if self.use_background:
            ...
            def fetch_bg_link(lx, ly, lz):
                ...
                mask = lnk >= 0
                results[mask] = self.background_data[
                    lnk[mask].long(), lz[mask]
                ]
                return results
            ...
            for i in range(n_steps):
                ...
                layerid = min(
                    int(normalized_inv_radius), self.background_nlayers - 2
                )
    ...
    def _volume_render_gradcheck_nvol_lerp(
        ...
        while good_indices.numel() > 0:
            ...
            new_total_alpha = torch.clamp_max(
                total_alpha_sub + delta_alpha, 1.0
            )
    ...
    def volume_render_image(
        ...
        if (
            ...
            for batch_start in range(
                0, camera.height * camera.width, batch_size
            ):
        ...
        with torch.no_grad():
            ...
            if use_weight_thresh:
                ...
                max_wt_grid = torch.zeros(
                    reso, dtype=torch.float32, device=device
                )
                ...
                if (
                    ...
                    weight_thresh_bounded = (
                        torch.topk(
                            max_wt_grid.view(-1), k=max_elements, sorted=False
                        )
                        .values.min()
                        .item()
                    )
                    ...
                    print(
                        " Readjusted weight thresh to fit to memory:",
                        weight_thresh,
                    )
                    sample_vals_mask = max_wt_grid >= weight_thresh
                del max_wt_grid
                ...
                if (
                    ...
                    print(
                        " Readjusted sigma thresh to fit to memory:",
                        sigma_thresh,
                    )
                    sample_vals_mask = sample_vals_density >= sigma_thresh
            ...
            if use_z_order:
                ...
                inv_morton[morton] = torch.arange(
                    morton.size(0), dtype=morton.dtype
                )
                inv_idx = inv_morton[sample_vals_mask]
                init_links = torch.full(
                    (sample_vals_mask.size(0),),
                    fill_value=-1,
                    dtype=torch.int32,
                )
                init_links[inv_idx] = torch.arange(
                    inv_idx.size(0), dtype=torch.int32
                )
            else:
                init_links = (
                    torch.cumsum(
                        sample_vals_mask.to(torch.int32), dim=-1
                    ).int()
                    - 1
                )
            ...
            print(
                "density", sample_vals_density.shape, sample_vals_density.dtype
            )
    ...
    def resize(self, basis_dim: int):
        ...
        new_data = torch.zeros(
            (old_data.size(0), 3 * basis_dim + 1), device="cpu"
        )
        if shrinking:
    ...
    def world2grid(self, points):
        ...
        return torch.addcmul(
            offset.to(device=points.device),
            points,
    ...
    def load(cls, path: str, device: Union[torch.device, str] = "cpu"):
        ...
        radius = (
            z.f.radius.tolist() if "radius" in z.files else [1.0, 1.0, 1.0]
        )
        center = (
            z.f.center.tolist() if "center" in z.files else [0.0, 0.0, 0.0]
        )
        grid = cls(
            1,
            radius=radius,
    ...
    def to_svox1(self, device: Union[torch.device, str, None] = None):
        ...
        dtype = torch.float32
        X = (
            torch.arange(curr_reso[0], dtype=dtype, device=device) + 0.5
        ) / curr_reso[0]
        Y = (
            torch.arange(curr_reso[1], dtype=dtype, device=device) + 0.5
        ) / curr_reso[0]
        Z = (
            torch.arange(curr_reso[2], dtype=dtype, device=device) + 0.5
        ) / curr_reso[0]
        X, Y, Z = torch.meshgrid(X, Y, Z)
        points = torch.stack((X, Y, Z), dim=-1).view(-1, 3)
    ...
    def tv_color(
        ...
        start_dim = (
            start_dim + self.sh_data.size(1) if start_dim < 0 else start_dim
        )
        return _TotalVariationFunction.apply(
            self.sh_data,
            self.links,
    ...
    def inplace_l2_color_grad(
        ...
        with torch.no_grad():
            if end_dim is None:
                end_dim = self.sh_data.size(1)
            end_dim = (
                end_dim + self.sh_data.size(1) if end_dim < 0 else end_dim
            )
            start_dim = (
                start_dim + self.sh_data.size(1)
                if start_dim < 0
                else start_dim
            )
            ...
            if self.sparse_sh_grad_indexer is None:
                ...
                grad[:, start_dim:end_dim] += (
                    scale * self.sh_data[:, start_dim:end_dim]
                )
            else:
    ...
    def optim_density_step(
        ...
        if optim == "rmsprop":
            ...
            _C.sgd_step(
                self.density_data.data, self.density_data.grad, indexer, lr, lr
            )
        else:
    ...
    def optim_basis_step(
        ...
        if optim == "rmsprop":
            if (
                self.basis_rms is None
                or self.basis_rms.shape != self.basis_data.shape
            ):
                del self.basis_rms
            ...
            self.basis_data.data.addcdiv_(
                self.basis_data.grad, denom, value=-lr
            )
        elif optim == "sgd":
    ...
    def _grid_size(self):
        return torch.tensor(
            self.links.shape, device="cpu", dtype=torch.float32
        )
    ...
    def _maybe_convert_sparse_grad_indexer(self, sh=False, bg=False):
        ...
        """
        indexer = (
            self.sparse_sh_grad_indexer if sh else self.sparse_grad_indexer
        )
        if bg:
            indexer = self.sparse_background_indexer
        ...
        if (
            ...
            indexer = torch.nonzero(
                indexer.flatten(), as_tuple=False
            ).flatten()
        return indexer
    ...
    def _get_rand_cells(
        ...
        if sparse_frac < 1.0 or force:
            ...
            grid_size = (
                self.links.size(0) * self.links.size(1) * self.links.size(2)
            )
    ...
    def _get_rand_cells_background(
    ...
    def reinit_learned_bases(
        ...
        if init_type == "sh":
            ...
            sph_vals = (
                utils.eval_sg_at_dirs(sg_lambdas, sg_dirvecs, points)
                * sg_sigmas
            )
            ...
            fourier_scale = 1.0 / torch.sqrt(
                2 * np.pi
                - torch.cos(fourier_freqs)
                * torch.sin(fourier_freqs)
                / fourier_freqs
            )
